...
public sealed class DirtyWorkCheckpointPolicy(GitProcess git, TimeProvider timeProvider)
{
    ...
    {
        if (invocation.Effect != ToolEffect.WorkspaceMutation)
        {
            return null;
        }
        if (IsRequired(context.State.WorkspacePath))
        {
            return Blocked();
        }
        if (timeProvider.GetUtcNow() - context.State.LastContinuityAt < _interval)
        {
            return null;
        }
