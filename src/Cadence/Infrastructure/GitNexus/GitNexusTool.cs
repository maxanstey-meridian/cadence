...
)
{
    ...
    > _execute =
        execute
        ?? ((startInfo, cancellationToken) => ExecuteProcessAsync(startInfo, cancellationToken));
    ...
    internal static async Task<GitNexusProcessResult> ExecuteProcessAsync(
        ProcessStartInfo startInfo,
        CancellationToken cancellationToken,
        TimeSpan? timeoutAfter = null
    )
    {
        var executionTimeout = timeoutAfter ?? _timeout;
        ...
        catch (OperationCanceledException)
        {
            ...
            throw new TimeoutException(
                $"GitNexus '{startInfo.ArgumentList[0]}' exceeded {executionTimeout}."
            );
        }
